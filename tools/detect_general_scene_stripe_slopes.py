...
The detector intentionally uses two scores:
  1. thin high-alpha lines: high-alpha pixels concentrated on b = row - a * col
     line bins. One slope family is selected.
  2. broad offset bands: the previous angle sweep destriping score. For each
     positive slope a, group pixels by b = row - a * col, estimate the line-wise
...
Outputs:
  ...
  - positive_slope_search_all_candidates.csv
  - positive_slope_search_selected_primary.csv
...
class SlopeDetectionConfig:
    ...
    # Positive slopes are searched as angles because equal slope steps bias
    # steep directions. 0 < theta < 90 deg corresponds to positive slopes.
    ...
    #   alpha pixels on b = row - a * col line bins.
    ...
    # Same basic line grouping as the earlier code:
...
def make_positive_slope_candidates(cfg: SlopeDetectionConfig) -> pd.DataFrame:
    ...
    return pd.DataFrame({"angle_deg": angles, "slope": slopes})
...
def search_thin_positive_slopes(alpha: np.ndarray, valid_mask: np.ndarray, cfg: SlopeDetectionConfig) -> tuple[pd.DataFrame, dict]:
    ...
    candidates = make_positive_slope_candidates(cfg)
    ...
    for i, row in candidates.iterrows():
        ...
        rows.append(thin_slope_search_score(alpha, valid_mask, high_mask, float(row["slope"]), cfg))
...
def build_six_direction_table(selected_primary: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, src in selected_primary.iterrows():
        ...
        orth_slope_param = 1.0 / a
        orth_signed_slope = -orth_slope_param
        ...
        rows.append(
            {
                **common,
                ...
                "direction_key_for_existing_code": "y_minus_x",
                "slope_parameter_for_existing_code": a,
                "equation_form": f"row = {a:.8f} * col + b",
            }
        )
        rows.append(
            {
                **common,
                ...
                "direction_key_for_existing_code": "y_plus_x",
                "slope_parameter_for_existing_code": orth_slope_param,
                "equation_form": f"row = -{orth_slope_param:.8f} * col + b",
            }
        )
...
def plot_score_curve(search_table: pd.DataFrame, selected_primary: pd.DataFrame, output_path: Path) -> None:
    ...
    ax.set_xlabel("angle theta = atan(slope) [deg]")
    ax.set_ylabel("score = robust_std_before - robust_std_after")
    ax.set_title("Positive slope search")
...
def plot_detected_direction_overlay(
    ...
    ax.set_title("Detected positive slopes and orthogonal directions")
...
def run_detection(cfg: SlopeDetectionConfig) -> dict:
    ...
    if cfg.verbose:
        print(f"\nSelected positive primary slopes (thin {cfg.thin_top_k} + broad {cfg.broad_top_k}):")
        if len(selected_primary) == 0:
            print("  none")
        else:
...
def parse_args() -> SlopeDetectionConfig:
    parser = argparse.ArgumentParser(description="Detect positive stripe slopes and their orthogonal directions.")
