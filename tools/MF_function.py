...
# regex pattern to extract wavelength from column names like "wave_2300.0nm"
...
# load spectra from a CSV file with columns `y`, `x`, and `wave_XXXnm`. The function returns the DataFrame, the wavelengths array, and the spectra array. It checks for the required columns and that the spectra contain finite values.
...
# convert spectra with `(y, x)` coordinates into an image cube. The function takes a DataFrame with `y` and `x` columns, a 2D array of spectra, and an optional fill value for missing pixels. It returns the cube array, the sorted unique y coordinates, and the sorted unique x coordinates.
def spectra_to_cube(
    df: pd.DataFrame,
    spectra: np.ndarray,
...
# flatten a cube of shape `(height, width, n_bands)` into a 2D array of shape `(height * width, n_bands)`. The function reshapes the cube so that each row corresponds to the spectrum of one pixel.
...
# create a boolean mask for selecting wavelength bands based on minimum and maximum wavelength limits and optional exclusion ranges. The function returns a boolean array that can be used to index the wavelengths and spectra.
def band_mask(
...
# select wavelength bands from a cube using the band mask. The function takes a cube, the corresponding wavelengths, and the selection criteria, and returns the selected cube, wavelengths, and the mask used for selection.
def select_bands(
    cube: np.ndarray,
    wavelengths: np.ndarray,
    wl_min: float,
    ...
    """Select wavelength bands from a cube."""
...
# estimate the mean and covariance of the background spectra, optionally using a mask to select which spectra to include. The function checks that there are valid spectra available and applies regularization to the covariance matrix.
...
def alpha_to_map(alpha: np.ndarray, height: int, width: int) -> np.ndarray:
    """Reshape flattened alpha values to an image map."""
...
def plot_iterative_mf_result(result: dict[str, object]) -> None:
    """Plot the main outputs of `iterative_matched_filter_map_from_cube`."""
