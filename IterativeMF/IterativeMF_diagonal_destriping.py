#!/usr/bin/env python3
"""Improved Iterative Matched Filter with optional diagonal destriping.

Purpose
-------
The workflow combines two ideas:

1. Iterative Matched Filter processing for methane plume detection.
2. Directional destriping on MF alpha maps along image diagonals
   such as ``row - col = const``.

The module is safe to import: no analysis is executed at import time.
Run it as a script to execute the default workflow, or import the helper
...
# ROI CSV format: columns = y, x, wave_XXXXnm, wave_XXXXnm, ...
ROI_CSV = Path("all_roi_spectra200x200.csv")

# MODTRAN CSV format: columns = wavelength, 0.0, 0.5, 1.0, ...
MODTRAN_CSV = Path("CH4c.csv")

OUTPUT_DIR = Path("outputs_improved_diagonal_destripe")

# Wavelength and UAS settings
WL_MIN = 2100.0
WL_MAX = 2450.0
FWHM_NM = 12.5

# Alpha range used for UAS estimation.
# The default follows the low-alpha setting used in the original notebook.
UAS_ALPHA_MIN = 0.0
UAS_ALPHA_MAX = 0.5
...
# Recommended diagonal-destriping parameters
DEFAULT_DESTRIPE_PARAMS = {
    # Direction of line grouping:
    # "y_minus_x" groups lines parallel to y=x, where row - col = constant.
    # Use "y_plus_x" to test lines parallel to y=-x.
    "direction": "y_minus_x",

    # Statistic used to estimate each diagonal-line offset.
    # Available methods: "median", "mean", "trimmed_mean", "mode".
    "method": "median",

    # Minimum number of pixels required to estimate an offset for one diagonal line.
    "min_pixels_per_line": 5,

    # True subtracts line_stat - global_stat, which keeps the global alpha level stable.
    # False subtracts line_stat directly.
    "preserve_global_stat": True,

    # Half-window for median smoothing of neighboring diagonal offsets.
    # Try 0, 1, or 2 first.
    "smooth_half_window": 0,

    # Pixel exclusion mode for estimating stripe offsets.
    # Available modes: "none", "robust_high", "previous_plume", "previous_plume_or_high".
    # For "final_only", previous_plume is interpreted as the final plume mask from the MF loop.
    "exclude_mode": "robust_high",
    "exclude_nsigma": 4.0,

    # If too few pixels remain on a diagonal after exclusion, retry using valid pixels.
    "fallback_to_valid": True,

    # Parameter for trimmed_mean.
    "trim_fraction": 0.1,

    # Parameter for mode. The mode of continuous values is approximated by a histogram.
    "mode_bins": 64,

    # Source alpha map used for plume thresholding during each_iter destriping.
    # "corrected" is recommended. "raw" saves correction products but updates masks using raw alpha.
...
# destripe_when:
#   "none"       : no destriping
#   "final_only" : apply destriping only to the alpha map after the MF loop
#   "each_iter"  : apply destriping during each iteration before plume thresholding
#   [3, 4, 5]    : apply destriping only at selected iteration numbers
EXPERIMENTS = {
...
DEFAULT_CASES_TO_SAVE = ("median_final_only", "median_each_iter")
...
# compute_uas_log_slope fits log(spectrum) = intercept - UAS * alpha, which is equivalent to spectrum = exp(intercept) * exp(-UAS * alpha).
...
# estimate background mean and covariance from selected pixels, with diagonal regularization.
...
# make_methane_target follows the convention used in the original notebook, where positive alpha corresponds to negative target values.
...
# compute the MF alpha map using the current background pixels, mean, covariance, and target spectrum.
...
# median + nsigma * 1.4826*MAD threshold for plume detection from alpha values.
...
# return line IDs for directional line grouping. Image coordinate convention: row = y, positive downward; col = x, positive rightward.
...
# Compute a median-smoothed 1D array while ignoring NaN. Used for smoothing estimated stripe offsets along neighboring lines.
...
# Compute a 1D statistic for stripe offset estimation. Available methods: "median", "mean", "trimmed_mean", "mode".
def statistic_1d(
    values: np.ndarray,
    method: str = "median",
...
# build a boolean mask of pixels to exclude from stripe estimation based on alpha values and/or a plume mask. The exclude_mode parameter controls the exclusion criteria.
...
# estimate stripe offsets for each directional line and subtract them from the alpha map. The method parameter controls the statistic used for offset estimation, and preserve_global_stat controls whether to keep the global alpha level stable after destriping.
...
# run the iterative matched filter loop, optionally applying diagonal destriping at specified iterations. Returns a history of results for each iteration, including alpha maps, masks, thresholds, and metadata.
def run_iterative_mf_with_optional_destriping(
    ...
    for it in range(1, n_iter + 1):
        ...
        line_table = pd.DataFrame()
        exclude_meta = {"exclude_mode": "none", "n_excluded": 0}
        ...
        if apply_now:
            ...
            out = destripe_by_directional_lines(
                ...
                direction=destripe_params.get("direction", "y_minus_x"),
        ...
        exclude_meta_history.append(exclude_meta.copy())
    ...
    if destripe_when == "final_only":
        ...
        exclude_meta_history[-1] = exclude_meta.copy()
...
def summarize_results_table(results: dict[str, dict]) -> pd.DataFrame:
    ...
    for name, res in results.items():
        ...
        rows.append({
            ...
            "method": None if res["destripe_params"] is None else res["destripe_params"].get("method"),
            "direction": None if res["destripe_params"] is None else res["destripe_params"].get("direction"),
            "smooth_half_window": None if res["destripe_params"] is None else res["destripe_params"].get("smooth_half_window"),
...
def plot_single_result(res: dict, title_prefix: str = "result"):
    ...
    if line_table is not None and len(line_table) > 0:
        plt.figure(figsize=(8, 4))
        plt.plot(line_table["line_id"], line_table["stripe_offset_subtracted"], marker=".", linewidth=1)
...
def save_case_outputs(
    ...
    line_ids = line_id_map(raw.shape, direction=result["destripe_params"].get("direction", "y_minus_x") if result["destripe_params"] else "y_minus_x")
    ...
    pixel_df = pd.DataFrame({
        ...
        "y": y_values,
        "x": x_values,
        "line_id": line_ids.ravel(),
        "is_valid": valid.ravel(),
        "alpha_raw": raw.ravel(),
        "stripe_offset_subtracted": stripe.ravel(),
...
# 8. Full workflow / command line interface


# parse a comma-separated list of values from a command-line argument, returning None if the input is None or empty. This is used for flexible specification of experiment names to run from the command line.
...
def run_workflow(
    ...
    reg: float = REG,
    rcond: float = RCOND,
    nodata_values: Optional[Sequence[float]] = NODATA_VALUES,
    ...
    cases_to_save: Sequence[str] = DEFAULT_CASES_TO_SAVE,
    show_plots: bool = False,
    verbose: bool = True,
) -> dict: # run the full workflow with the specified parameters, including loading data, computing UAS, running iterative matched filter with optional destriping for multiple experiments, and saving results. Returns a dictionary containing the loaded data and results for further analysis.
    roi_csv = Path(roi_csv)
    modtran_csv = Path(modtran_csv)
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if experiment_names is not None:
        keep = set(experiment_names)
        missing = sorted(keep - set(experiments))
        if missing:
            raise KeyError(f"Unknown experiment name(s): {missing}")
        experiments = {name: cfg for name, cfg in experiments.items() if name in keep}

    if verbose:
        print("Loading ROI spectra...")
    ...
    if show_plots:
        ...
        try:
            ...
            plot_map(rgb, title="RGB preview", vmin=0, vmax=1, colorbar_label=None)
        except Exception as exc:
            print(f"RGB preview skipped: {exc}")

    if verbose:
        print("Loading MODTRAN spectra and computing UAS...")
    ...
    if verbose:
        ...
        print(f"Selected wavelength range: {wave_sel[0]:.2f} - {wave_sel[-1]:.2f} nm")
    ...
    for name, cfg in experiments.items():
        ...
        result = run_iterative_mf_with_optional_destriping(
            cube=cube_sel,
            uas=uas_sel,
            valid_mask=valid_mask,
    ...
    for case_name in cases_to_save:
        if case_name in results:
            ...
            if verbose:
                print(f"Saved outputs for case: {case_name}")

    if show_plots:
        plot_threshold_histories(results)

        plot_names = [name for name in PRIMARY_PLOT_NAMES if name in results]
        if plot_names:
            plot_experiment_grid(results, names=plot_names)

        if "median_final_only" in results:
            plot_single_result(results["median_final_only"], title_prefix="median_final_only")
        if "median_each_iter" in results:
            plot_single_result(results["median_each_iter"], title_prefix="median_each_iter")
    ...
    return {
        ...
        "wavelengths_selected": wave_sel,
        "uas_selected": uas_sel,
        "valid_mask": valid_mask,
        "results": results,
        "summary": summary_df,
        "output_dir": output_dir,
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Iterative MF with optional y=x diagonal destriping."
    )
    ...
    parser.add_argument("--wl-min", type=float, default=WL_MIN, help="Minimum wavelength used for MF.")
    parser.add_argument("--wl-max", type=float, default=WL_MAX, help="Maximum wavelength used for MF.")
    parser.add_argument("--fwhm-nm", type=float, default=FWHM_NM, help="Sensor FWHM used for Gaussian SRF resampling.")

    parser.add_argument("--uas-alpha-min", type=float, default=UAS_ALPHA_MIN, help="Minimum alpha for UAS log-slope fitting.")
    parser.add_argument("--uas-alpha-max", type=float, default=UAS_ALPHA_MAX, help="Maximum alpha for UAS log-slope fitting.")

    parser.add_argument("--n-iter", type=int, default=N_ITER, help="Maximum number of Iterative MF iterations.")
    parser.add_argument("--nsigma", type=float, default=NSIGMA, help="Robust threshold multiplier.")
    parser.add_argument("--reg", type=float, default=REG, help="Covariance regularization strength.")
    parser.add_argument("--rcond", type=float, default=RCOND, help="Pseudo-inverse rcond.")

    parser.add_argument(
        "--experiments",
        type=str,
        default=None,
        help="Comma-separated experiment names to run. Default: run all predefined experiments.",
    )
    parser.add_argument(
        "--cases-to-save",
        type=str,
        default=",".join(DEFAULT_CASES_TO_SAVE),
        help="Comma-separated case names for detailed per-pixel output files.",
    )
    parser.add_argument(
        "--show-plots",
        action="store_true",
        help="Show diagnostic plots interactively. Outputs are saved even when this is false.",
    )
    parser.add_argument(
        "--allow-nonpositive",
        action="store_true",
        help="Do not require selected-band values to be positive.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
...
def main(argv: Optional[Sequence[str]] = None) -> dict:
    ...
    return run_workflow(
        ...
        reg=args.reg,
        rcond=args.rcond,
        require_positive=not args.allow_nonpositive,
        experiment_names=parse_csv_list(args.experiments),
        cases_to_save=parse_csv_list(args.cases_to_save) or list(DEFAULT_CASES_TO_SAVE),
