...
def make_default_destripe_params(nsigma: float = NSIGMA) -> dict:
    return {
        # Direction order. First correct y=x-parallel lines, then y=-x-parallel lines.
        "directions": ["y_minus_x", "y_plus_x"],

        # Statistic used to estimate a stripe offset per line.
        "method": "median",

        # Minimum number of pixels required to estimate the offset of one line.
        "min_pixels_per_line": 5,

        # If True, subtract line_stat - global_stat so that the global alpha level is preserved.
        "preserve_global_stat": True,

        # Median smoothing half-window for neighboring line offsets. Try 0, 1, or 2.
        ...
        # Recompute the exclusion mask after each directional correction.
        "recompute_exclude_each_direction": True,

        # If too few pixels remain on a line after exclusion, estimate from all valid pixels on that line.
        "fallback_to_valid": True,

        # Parameters for trimmed_mean and mode.
        "trim_fraction": 0.1,
        "mode_bins": 64,

        # If destriping is performed inside the iteration loop, use the corrected alpha map for thresholding.
        "threshold_source": "corrected",
    }

# return the default experiment set used by the notebook. Each experiment is a dict with keys:
def make_experiments(nsigma: float = NSIGMA) -> dict[str, dict]:
    ...
    return {
        ...
        # One-direction cases kept for comparison with the previous module.
        ...
        # Alternative statistics for comparison.
        "mean_yx_then_ynegx_each_iter": {
            "destripe_when": "each_iter",
            "destripe_params": {**default, "method": "mean"},
        },
        "trimmed_mean_yx_then_ynegx_each_iter": {
            "destripe_when": "each_iter",
            "destripe_params": {**default, "method": "trimmed_mean"},
        },
        "mode_yx_then_ynegx_each_iter": {
            "destripe_when": "each_iter",
            "destripe_params": {**default, "method": "mode", "mode_bins": 64},
        },
        # Two-direction correction only after iteration 3.
...
# Workflow helpers and command line interface
...
def run_experiments(
    ...
    if experiment_names is not None:
        missing = [name for name in experiment_names if name not in experiments]
        if missing:
            raise ValueError(f"Unknown experiment names: {missing}")
        experiments = {name: experiments[name] for name in experiment_names}
...
def save_workflow_outputs(
    ...
    wave_sel: np.ndarray,
    uas_sel: np.ndarray,
    cases_to_save: Optional[Sequence[str]] = None,
) -> pd.DataFrame:
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    if cases_to_save is None:
        cases_to_save = [
            "median_yx_final_only",
            "median_yx_then_ynegx_final_only",
            "median_yx_then_ynegx_each_iter",
    ...
    for case_name in cases_to_save:
        if case_name in results:
            ...
            print(f"Saved case outputs: {case_name}")
        else:
...
def run_workflow(
    ...
    results = run_experiments(
        cube_sel=prepared["cube_sel"],
        uas_sel=prepared["uas_sel"],
    ...
    if show_plots:
        ...
        for name in ["median_yx_then_ynegx_final_only", "median_yx_then_ynegx_each_iter"]:
            if name in results:
                plot_single_result(results[name], title_prefix=name)

    summary_df = save_workflow_outputs(
        results=results,
        output_dir=output_dir,
        ys=prepared["ys"],
        xs=prepared["xs"],
        wave_sel=prepared["wave_sel"],
        uas_sel=prepared["uas_sel"],
        cases_to_save=cases_to_save,
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Iterative MF with optional two-direction diagonal destriping."
    )
    parser.add_argument("--roi-csv", type=Path, default=ROI_CSV, help="ROI spectra CSV path.")
    parser.add_argument("--modtran-csv", type=Path, default=MODTRAN_CSV, help="MODTRAN CH4 CSV path.")
    ...
    parser.add_argument("--uas-alpha-min", type=float, default=UAS_ALPHA_MIN, help="Minimum alpha for UAS log-slope fit.")
    parser.add_argument("--uas-alpha-max", type=float, default=UAS_ALPHA_MAX, help="Maximum alpha for UAS log-slope fit.")
    ...
    parser.add_argument("--reg", type=float, default=REG, help="Covariance regularization coefficient.")
    parser.add_argument("--rcond", type=float, default=RCOND, help="Pseudo-inverse rcond.")
    parser.add_argument(
        "--experiments",
        type=str,
        default=None,
        help="Comma-separated experiment names. If omitted, all default experiments are run.",
    )
    parser.add_argument(
        "--save-cases",
        type=str,
        default=None,
        help="Comma-separated case names to save. If omitted, the recommended cases are saved.",
    )
    parser.add_argument("--show-plots", action="store_true", help="Display diagnostic plots.")
    parser.add_argument("--quiet", action="store_true", help="Reduce iteration logging.")
    ...
    run_workflow(
        ...
        cases_to_save=parse_csv_list(args.save_cases),
